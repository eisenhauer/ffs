...
cg_operator(dill_stream s, sm_ref expr, int need_assignable, cod_code descr)
{
	...
	dill_mark_label_type false_label = dill_alloc_label(s);
	...
	return ret_op;
    case  op_leq:{
	dill_mark_label_type true_label = dill_alloc_label(s);
	dill_seti(s, result, 0);	/* op_i_seti */
	switch (op_type) {
	case DILL_I:
...
static void cg_selection_statement(dill_stream s, sm_ref stmt, cod_code descr)
{
    dill_mark_label_type else_label = dill_alloc_label(s);
...
static void cg_iteration_statement(dill_stream s, sm_ref stmt, cod_code descr)
{
    dill_mark_label_type begin_label = dill_alloc_label(s), end_label = dill_alloc_label(s);
